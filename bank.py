...
class Account:
    def __init__(self, name, balance, card_file):
        ...
        if(card_file):
            ...
            self.card_file = name + '.card'
            f = open(self.card_file, 'w')
            f.write(str(self.card_number))
...
def authenticate(f, conn):
    ...
    counter = requets.counter
    conn.send(json.dumps({'counter' : counter + 1}))
    return counter
    
def handle_request(f, conn, counter):
    ciphertext = conn.recv(BUFFER_SIZE)
    if not (ciphertext):
        print "invalid data"
        return 0
    ...
    if(request.counter != counter + 2):
        ...
        #response = create(request.card_number, request.name, request.amount)
    #elif(request.operation == "deposit"):
        #response = deposit(request.card_number, request.amount)
    #elif(request.operation == "withdraw"):
        #response = withdraw(request.card_number, request.amount)
    #elif(request.operation == "getinfo"):
        #response = getinfo(request.card_number)
    #else:
       # return 0
    
    #return response
    return {"counter":request.counter + 1, "summary": "{\"stuff\":\"happened\"}"}
...
while True:
    ...
    response = handle_request(f, conn, counter)
    if(response):
